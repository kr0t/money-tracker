...
5. **Привязка D1 к Pages:**
   ...
   - Нажмите **Save**.
6. **Настройка PIN-кода (авторизация):**
   - В настройках проекта Pages перейдите в **Settings** -> **Environment variables** (или **Variables and Secrets**).
   - Добавьте переменную (или секрет):
     - Variable name: `AUTH_PIN`
     - Value: `ваш_пин_или_пароль` (например, `1234` или любой пароль)
   - *(Опционально)* Можно также добавить `AUTH_SECRET` (любая случайная длинная строка для подписи токенов).
   - Нажмите **Save**.
...
- Для локальной разработки скопируйте `.dev.vars.example` в `.dev.vars`:
  ```bash
  cp .dev.vars.example .dev.vars
  ```
  По умолчанию для локальной разработки используется PIN `1234`.
